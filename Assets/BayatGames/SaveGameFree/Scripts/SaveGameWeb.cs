...
namespace BayatGames.SaveGameFree
{
	...
	public class SaveGameWeb
    {
	    ...
	    public SaveGameWeb(string username, string password, string url, bool encode) : this(username, password, url,
            encode, DefaultEncodePassword)
        {
        }
	    ...
	    public SaveGameWeb(string username, string password, string url, bool encode, string encodePassword) : this(
            username, password, url, encode, encodePassword, DefaultSerializer)
        {
        }
	    ...
	    public SaveGameWeb(string username, string password, string url, bool encode, string encodePassword,
            ISaveGameSerializer serializer) : this(username, password, url, encode, encodePassword, serializer,
            DefaultEncoder)
        {
        }
	    ...
	    public SaveGameWeb(string username, string password, string url, bool encode, string encodePassword,
            ISaveGameSerializer serializer, ISaveGameEncoder encoder) : this(username, password, url, encode,
            encodePassword, serializer, encoder, DefaultEncoding)
        {
        }
	    ...
	    public SaveGameWeb(string username, string password, string url, bool encode, string encodePassword,
            ISaveGameSerializer serializer, ISaveGameEncoder encoder, Encoding encoding)
        {
            m_Username = username;
            m_Password = password;
            m_URL = url;
	    ...
	    public static string DefaultUsername { get; set; } = "savegamefree";

	    /// <summary>
	    ///     Gets or sets the default password.
	    /// </summary>
	    /// <value>The default password.</value>
	    public static string DefaultPassword { get; set; } = "$@ve#game%free";

	    /// <summary>
	    ///     Gets or sets the default UR.
	    /// </summary>
	    /// <value>The default UR.</value>
	    public static string DefaultURL { get; set; } = "http://www.example.com";
	    ...
	    public static bool DefaultEncode { get; set; } = false;

	    /// <summary>
	    ///     Gets or sets the default encode password.
	    /// </summary>
	    /// <value>The default encode password.</value>
	    public static string DefaultEncodePassword { get; set; } = "h@e#ll$o%^";
	    ...
	    public static ISaveGameSerializer DefaultSerializer
        {
            get
            {
                if (m_DefaultSerializer == null) m_DefaultSerializer = new SaveGameJsonSerializer();
                return m_DefaultSerializer;
            }
            set { m_DefaultSerializer = value; }
        }
	    ...
	    public static ISaveGameEncoder DefaultEncoder
        {
            get
            {
                if (m_DefaultEncoder == null) m_DefaultEncoder = new SaveGameSimpleEncoder();
                return m_DefaultEncoder;
            }
            set { m_DefaultEncoder = value; }
        }
	    ...
	    public static Encoding DefaultEncoding
        {
            get
            {
                if (m_DefaultEncoding == null) m_DefaultEncoding = Encoding.UTF8;
	    ...
	    public virtual ISaveGameSerializer Serializer
        {
            get
            {
                if (m_Serializer == null) m_Serializer = new SaveGameJsonSerializer();
                return m_Serializer;
            }
	    ...
	    public virtual ISaveGameEncoder Encoder
        {
            get
            {
                if (m_Encoder == null) m_Encoder = new SaveGameSimpleEncoder();
                return m_Encoder;
            }
            set { m_Encoder = value; }
        }
	    ...
	    public virtual Encoding Encoding
        {
            get
            {
                if (m_Encoding == null) m_Encoding = Encoding.UTF8;
                return m_Encoding;
            }
	    ...
	    public virtual IEnumerator Save<T>(string identifier, T obj)
        {
            ...
            if (Encode) data = Encoder.Encode(data, EncodePassword);
	    ...
	    public virtual T Load<T>(string identifier, T defaultValue)
        {
            ...
            if (!m_IsError && !string.IsNullOrEmpty(m_Request.downloadHandler.text))
            {
                ...
                if (Encode) data = Encoder.Decode(data, EncodePassword);
                var memoryStream = new MemoryStream(Encoding.GetBytes(data));
	    ...
	    public virtual IEnumerator Send(string identifier, string data, string action)
        {
            ...
            if (!string.IsNullOrEmpty(data)) formFields.Add("data", data);
            if (!string.IsNullOrEmpty(Password)) formFields.Add("password", Password);
